...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `RamaHttp.swiftmodule/`
...
//      `import RamaHttp` above would fail at compile time.
//
//   2. The static archive `libRamaHttp.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "RamaHttp" }` and
//      `flattenPackage = "io.github.kotlinmania.ramahttp"` configuration in
...
//      via the `RamaHttpLibrary` product.
//
...
final class RamaHttpExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "RamaHttp swift module imported cleanly")
    }
}
